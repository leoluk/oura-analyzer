...
/** Build an aggregated area mark, using LOESS or Plot.windowY depending on aggregation type. */
function aggArea(data, agg, k, xKey, yKey, areaOptions, {loessSpan = 30} = {}) {
    if (agg === "loess") {
        return Plot.areaY(loessSmooth(data, xKey, yKey, loessSpan / 100), {x: xKey, y: yKey, ...areaOptions, curve: "natural"});
    }
    return Plot.areaY(data, Plot.windowY({k, anchor: "end", reduce: agg}, {x: xKey, y: yKey, ...areaOptions}));
}

/** Generate Plot marks for user-defined markers (vertical dashed lines + labels). */
function markerMarks(markers, data, xKey = "date") {
    const dates = data.map(d => d[xKey]).filter(d => d != null);
    if (dates.length === 0) return [];
    const min = dates[0], max = dates[dates.length - 1];
    return markers.filter(m => m.date >= min && m.date <= max).flatMap(m => [
        Plot.ruleX([m.date], {stroke: m.color, strokeWidth: 2, strokeDasharray: "4,4"}),
        Plot.text([m], {
            x: "date", text: "label", fill: m.color,
            fontSize: 10, dx: 4, textAnchor: "start", frameAnchor: "top", dy: 6,
        }),
    ]);
}
    ...
    _emitChange() {
        this.dispatchEvent(new CustomEvent('change', {
            detail: {
                ...
                showAUC: this.showAUC,
            }
        }));
    }

    updated(changedProperties) {
        const sliderChanged = changedProperties.has('kWindow') || changedProperties.has('loessSpan');
        const onlySliderChanged = sliderChanged &&
            [...changedProperties.keys()].every(k => k === 'kWindow' || k === 'loessSpan');
        if (onlySliderChanged) {
            clearTimeout(this._debounceTimer);
            this._debounceTimer = setTimeout(() => this._emitChange(), 250);
        } else {
            clearTimeout(this._debounceTimer);
            this._emitChange();
        }
        ...
        return html`
            ...
            ${this.plotType === 'line' ? (this.aggType === 'loess' ? html`
                <sl-range label="Span (${this.loessSpan}%)" min="1" max="100"
                          value=${this.loessSpan} @sl-input=${e => {
                              const val = +e.target.value;
                              e.target.label = 'Span (' + e.target.value + '%)';
                              clearTimeout(this._loessSpanTimer);
                              this._loessSpanTimer = setTimeout(() => this.loessSpan = val, 250);
                          }}>
                </sl-range>
                <sl-checkbox size="small"
                        @sl-change=${e => this.showAUC = e.target.checked}
                        .checked=${this.showAUC}>
                    Show AUC
                </sl-checkbox>
            ` : html`
                <sl-range label="Window (${this.kWindow}d)" min="1" max="60"
                          value=${this.kWindow} @sl-input=${e => {
                              const val = +e.target.value;
                              e.target.label = 'Window (' + e.target.value + 'd)';
                              clearTimeout(this._kWindowTimer);
                              this._kWindowTimer = setTimeout(() => this.kWindow = val, 250);
                          }}>
                </sl-range>
                <sl-checkbox size="small"
                ...
                <sl-checkbox size="small"
                        @sl-change=${e => this.showTrend = e.target.checked}
...
class OuraSleepGraph extends LitElement {
    ...
    constructor() {
        ...
        this._detailGraphs = [
            {id: 0, focus: 'average_hrv', agg: 'loess', k: 7, loessSpan: 30, curve: 'step', plotType: 'line', interval: 'month', yZero: true, showTrend: false, showAUC: false},
        ];
        this._nextId = 1;
        ...
        try {
            const stored = JSON.parse(localStorage.getItem('oura-markers') || '[]');
            this._markers = stored.map(m => ({...m, date: new Date(m.date)}));
            this._nextMarkerId = this._markers.reduce((max, m) => Math.max(max, m.id + 1), 0);
        } catch (e) { /* ignore corrupt data */ }
    }

    _saveMarkers() {
        localStorage.setItem('oura-markers', JSON.stringify(
            this._markers.map(m => ({...m, date: m.date.toISOString().slice(0, 10)}))
        ));
    }

    _addMarker(date, label, color) {
        this._markers = [...this._markers, {id: this._nextMarkerId++, date: new Date(date), label, color}];
        this._saveMarkers();
    }
    ...
    _updateDetailGraph(id, params) {
        this._detailGraphs = this._detailGraphs.map(g =>
            g.id === id ? {...g, focus: params.focus, agg: params.agg, k: params.k, loessSpan: params.loessSpan, curve: params.curve, plotType: params.plotType, interval: params.interval, yZero: params.yZero, showTrend: params.showTrend, showAUC: params.showAUC} : g
        );
    }
            ...
            <div class="graph" @input=${e => console.log(this.selected = e.target.value?.date)}>
                ${Plot.plot({
                    ...
                    marks: [
                        ...
                        ...(this._showMarkers ? markerMarks(this._markers, this.sleepData) : []),
                    ]
                })}
            </div>
        `;
                ...
                <div class="graph">
                    ${Plot.plot({
                        ...
                        marks: [
                            ...
                            graph.showTrend ? Plot.linearRegressionY(this.sleepData, {
                                x: "date",
                                y: graph.focus,
                                opacity: 0.3,
                            }) : null,
        ...
        return html`
            <div class="graph">
                ${Plot.plot({
                    ...
                    marks: [
                        graph.showAUC ? aggArea(this.sleepData, graph.agg, graph.k, "date", graph.focus, {
                            fill: "steelblue", fillOpacity: 0.15, curve: graph.curve,
                        }, {loessSpan: graph.loessSpan}) : null,
            ...
            <div class="graph">
                ${Plot.plot({
                    ...
                    marks: [
                        ...
                        ...(this._showMarkers ? markerMarks(this._markers, data) : []),
                    ]
                })}
            </div>
        `;
    ...
    render() {
        return html`
            <div class="main-controls">
                <sl-select size="small" label="Aggregation" value=${this._mainAgg}
                ...
                ${this._mainAgg === 'loess' ? html`
                    <sl-range label="Span (${this._mainLoessSpan}%)" min="1" max="100"
                              value=${this._mainLoessSpan} @sl-input=${e => {
                                  const val = +e.target.value;
                                  e.target.label = 'Span (' + e.target.value + '%)';
                                  clearTimeout(this._mainLoessSpanTimer);
                                  this._mainLoessSpanTimer = setTimeout(() => this._mainLoessSpan = val, 250);
                              }}>
                    </sl-range>
                ` : html`
                    <sl-range label="Rolling window (${this._mainK} days)" min="1" max="60"
                              value=${this._mainK} @sl-input=${e => {
                                  const val = +e.target.value;
                                  e.target.label = 'Rolling window (' + e.target.value + ' days)';
                                  clearTimeout(this._mainKTimer);
                                  this._mainKTimer = setTimeout(() => this._mainK = val, 250);
                              }}>
                    </sl-range>
                    <sl-checkbox
                ...
                <div class="detail-section">
                    <div class="detail-header">
                        <detail-selector
                                ...
                                .interval=${graph.interval}
                                .yZero=${graph.yZero}
                                .showTrend=${graph.showTrend}
                                .showAUC=${graph.showAUC}
            ...
            <div class="markers">
                ...
                <sl-checkbox size="small"
                    @sl-change=${e => this._showMarkers = e.target.checked}
                    .checked=${this._showMarkers}>
                    Show markers
                </sl-checkbox>
                ${this._markers.length > 0 ? html`
                    <div class="marker-list">
                        ${this._markers.map(m => html`
                            <span class="marker-item">
                                <span class="marker-swatch" style="background:${m.color}"></span>
                                ${m.date.toLocaleDateString()} ${m.label}
                                <sl-icon-button name="x-lg" label="Remove"
                                    style="font-size: 0.75em;"
                                    @click=${() => this._removeMarker(m.id)}></sl-icon-button>
                            </span>
                        `)}
                    </div>
                ` : ''}
                <div class="marker-form">
                    <sl-input size="small" type="date" label="Date" id="marker-date"></sl-input>
                    <sl-input size="small" label="Label" id="marker-label"></sl-input>
                    <sl-color-picker size="small" label="Color" value="#e03e3e" id="marker-color"></sl-color-picker>
                    <sl-button size="small" @click=${() => {
                        const root = this.shadowRoot;
                        const date = root.getElementById('marker-date').value;
                        const label = root.getElementById('marker-label').value;
                        const color = root.getElementById('marker-color').value;
                        if (date && label) {
                            this._addMarker(date, label, color);
                        }
                    }}>
                        <sl-icon slot="prefix" name="plus-lg"></sl-icon>
                        Add marker
